...
void testPaperCostsUseCanonicalContract()
{
    ...
    const BrokerFill fill = gateway.submitOrder("X", true, 2.0, 100.0, 1, "WP2");
    require(fill.success, "deterministic PAPER fill failed");
    requireNear(fill.fillPrice, 100.1, "PAPER slippage mismatch");
    requireNear(fill.filledQuantity, 2.0, "PAPER quantity mismatch");
    requireNear(fill.feePaid, 0.4004, "PAPER fee mismatch");
...
void testMalformedFillConfirmationFailsClosed()
{
    ...
    const BrokerFill first = gateway.submitOrder("X", true, 2.0, 100.0, 1, "WP2");
    require(first.success && first.partialFill,
            "partial PAPER fixture did not produce an open lifecycle");
    const auto before = gateway.orderLifecycle(first.orderId);
    require(before.has_value(), "partial lifecycle record is missing");
    const auto errorsBefore = gateway.totalApiErrors();

    BrokerFill malformed = first;
    malformed.filledQuantity = std::numeric_limits<double>::max();
    malformed.fillPrice = 100.0;
    malformed.feePaid = 0.0;
    malformed.fillTimestamp = 2;
    gateway.simulateFillConfirmation(malformed);

    const auto after = gateway.orderLifecycle(first.orderId);
