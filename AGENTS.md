...
1. Inspect Git state, this file, `README.md`, `ARCHITECTURE.md`, and `HANDOFF.md` before mutation.
...
6. Do not commit, push, publish, deploy, execute provider traffic, or enable any execution capability without separately explicit authorization.

## Architecture reminders

- The current site is a static React frontend with no backend behavior.
...
Report the exact source and documentation files changed, commands run, verification outcome, warnings, skipped checks, residual risks, Git status, and whether external/provider actions were intentionally not performed.
